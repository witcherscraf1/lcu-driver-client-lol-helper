...
input：
    ...
    wholeChart:所有数据，用excel表格生成而归一化处理后的数据。
...
def evaluate_function(ourSpareHero, ourChosenHero, enemyChosenHero,wholeChart):
    ...
    # 对于基本的评分，由胜率，登场率，ban率构成，这个比率均是可以调整的。
    # (附：胜率，ban率，登场率均已归一化)
    # winRateFactor: 胜率因子 0.6
    # appearFactor:   登场因子 0.2
    # BanFactor:        ban因子 0.2
    ...
    for i in range(len(ourSpareHero)):
        ...
        originalHeroScore.append(whole_score)       #初始英雄分数
    ...
    for i in range(len(companyHero)):
        for j in range(len(ourSpareHero)):
            if companyHero[i]==ourSpareHero[j]:
                companyHeroScore[i]=companyHeroScore[i]+companyHeroAddScore     #company分数做累加，每配合一个己方英雄就+0.2
    ...
    antiKey = dict(zip(ourSpareHero, antiEnemyHeroScore))     #生成一个我方备选英雄与相应克制分数的字典
...
function:更新我方备选英雄池
input:
    finalChoseHero：上一轮选择的英雄
    ourSpareHero：我方备选英雄池
    ourChosenHero：我方已选英雄
    wholeChart：excel数据表单中提取的list字典集
output：
    newOurSpareHero：去掉上一轮选择的英雄对应分路后更新的英雄池
    ourChosenHero：我方更新后的已选英雄
'''
